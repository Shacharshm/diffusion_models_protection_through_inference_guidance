...
class SLDPipeline(DiffusionPipeline):
    ...
    def ssim_per_pixel(self, img1, img2, window_size=11, C1=1e-4, C2=9e-4):
        """Compute SSIM per pixel between two images."""
        # Ensure images have the same shape
        assert img1.shape == img2.shape

        # Gaussian window (mean filter for simplicity)
        ...
        sigma1_sq = F.conv2d(img1 * img1, window, padding=window_size // 2, groups=img1.size(1)) - mu1 ** 2
        sigma2_sq = F.conv2d(img2 * img2, window, padding=window_size // 2, groups=img2.size(1)) - mu2 ** 2
        ...
        for i, t in enumerate(self.progress_bar(timesteps_tensor)):
            ...
            if do_classifier_free_guidance:
                ...
                if enable_safety_guidance:
                    ...
                    noise_away_from_unsafe_concepts = self.ssim_per_pixel(noise_pred_safety_concept, noise_pred_text)

                    # Equation 6
                    scale = torch.clamp(
                        torch.abs((noise_away_from_unsafe_concepts)) * sld_guidance_scale, max=1.)

                    # Equation 6
                    safety_concept_scale = torch.where(
                        (noise_away_from_unsafe_concepts) >= sld_threshold,
